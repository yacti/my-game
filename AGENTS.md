...
Always work directly in this repository's main checkout. Do not create or switch branches, and do not create worktrees: the team uses one shared working tree and version-control state.
...
- `ReplicatedStorage.Assets.PetSkins` contains the permanent appearance templates. Exact Model names are player-facing; `PetSkinID` and `Rarity` are their only gameplay attributes, while animation/normalization/orientation data lives in `PetSkinCatalog`.
...
- `PetSkinPackShopService`: receipt-idempotent 1/12 Pet Skin Pack grants. It uses the paid-random-item gate and the existing Seed Pack result/reveal queue; twelve results open in four 3-item batches.
- `PetSkinService`: server-authoritative permanent Tool consumption and apply/replace validation. `skinId` stays on the existing per-slot pet record through evolution/rejoin/rebirth; base `petId`, XP, stats, and money remain authoritative.
...
- `LeaderboardService`: the two global boards on `Workspace.Leaderboard` (`TopRolls` ranks `profile.Data.LifetimeCrateRolls`, `TopCash` ranks live `profile.Data.Currency`, hence its "RIGHT NOW" subheader). Durable truth is one `OrderedDataStore` per board; Studio uses `_dev`-suffixed stores. Traffic is shaped for this game's **8-player servers**, where naive per-server polling would cost a flat 25% of the experience-wide `GetSortedAsync` budget and 46% of each server's Write budget (a pool shared with ProfileStore): writes are **cutoff-gated** (only players at or above the published rank-100 score are written on the timer; everyone else is written on leave and at `BindToClose`), and reads go through **one shared MemoryStore snapshot per board** that a single lock-winning server rebuilds per period — so `GetSortedAsync` is a constant ~4/min game-wide at any CCU. A missing snapshot disables periodic writes entirely rather than falling back to writing everyone. Admins are excluded at the write path (not in Studio, or Play Solo boards would stay empty). `src/client/ui/LeaderboardController.luau` renders the shared list from `LeaderboardSync` by reusing row instances rather than rebuilding ~700 per refresh, and drives the per-player `YourStat` labels off `PlayerStateStore` so your own number updates instantly instead of on the 30s cadence. Config in `src/shared/Leaderboards.luau`.
...
Two guards keep a partial projection from narrowing the profile. A restore that cannot mint everything records the unrestored remainder, which every snapshot adds back on top of the live counts (see Inventory in Engineering Rules) — the remainder is added rather than the map being frozen, so items the player legitimately spends still decrement instead of being re-minted on the next respawn. And a restore that never clears `InventoryRestorePending` is retried by a bounded watchdog, because otherwise no snapshot runs again for the rest of the session — including the leave snapshot, which would lose that session's pets, currency, and placements as well as tools. Serializers must count every character-parented tool (`Inventory.ForEachTool`), never just `FindFirstChildWhichIsA("Tool")`: the cap counter counts them all, so a serializer that sees one under-counts and the snapshot deletes the difference.
...
Countable inventory tools have two limits. `Inventory.MAX_ITEMS` (250, counting Backpack + equipped) is the soft cap normal pickups and grants refuse at. `Inventory.HARD_MAX_ITEMS` (1500) is the physical ceiling: nothing may mint above it, whatever cap a caller asks for, because the per-spawn re-materialization can only rebuild that many tools and anything beyond it would be dropped on the next respawn. The utility shovel does not count because it is non-persisted utility gear.

`ToolGrantService` is the single chokepoint and has no bypass option — `maxItems` is clamped to `HARD_MAX_ITEMS` and a grant at the limit is refused (returns `false, "inventory-full"`) with a rate-limited notification and matching `inventory_grant_blocked` telemetry. Every caller that mints a tool must handle grant failure by destroying the orphaned tool when needed, and producers should leave the source recoverable — tree pickups keep the ground pile, patch harvest keeps the slot, `Processor`/`JamBarrel` dequeue only after a successful grant, Durian rolls keep the ticket, Endless claims do not advance progress, and paid receipts return `NotProcessedYet` so blocked rewards stay collectable later.

Grants above the 250 soft cap are deliberate and enumerated: the collect-all developer product (food, `Inventory.COLLECT_ALL_MAX_ITEMS`), the Seed/Durian/Endless pack grants (seeds), and Pet Skin Pack grants. All five pass `maxItems = Inventory.HARD_MAX_ITEMS`; normal pickups/grants still refuse above 250. If a new over-soft-cap path is added, list it here.

Per-spawn inventory re-materialization in `init.server.luau` (every `CharacterAdded`, deficit-based — see Persistence And Player Lifecycle) is bounded by the same ceiling. Anything the profile holds that a run could not mint — ceiling reached, or a tool template that failed to build — is recorded as an unrestored remainder and added back by every snapshot that writes an inventory field, so a partial restore can never narrow the profile. Any new code path that writes `FeedInventory`, `SeedInventory`, `CosmeticInventory`, or `FoodInventoryItems` from live tools must go through that merge.
