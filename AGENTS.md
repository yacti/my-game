...
Countable inventory tools are capped at `Inventory.MAX_ITEMS` (250, counting Backpack + equipped). The utility shovel does not count because it is non-persisted utility gear. `ToolGrantService` enforces the cap for single and bulk grants: a grant at the limit is refused (returns `false, "inventory-full"`) with a rate-limited notification. Every caller that mints a tool must handle grant failure by destroying the orphaned tool when needed, and food producers should leave the source recoverable — tree pickups keep the ground pile, patch harvest keeps the slot, and `Processor`/`JamBarrel` dequeue only after a successful grant so blocked outputs stay queued for later collection. Join-time inventory re-materialization in `init.server.luau` is bounded by the same cap, so a profile hoarding more than the cap materializes at most `Inventory.MAX_ITEMS` countable tools and the next snapshot persists the trimmed count.
