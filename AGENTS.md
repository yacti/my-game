...
Grants above the 250 soft cap are deliberate and enumerated: the collect-all developer product (food, `Inventory.COLLECT_ALL_MAX_ITEMS`) and the Seed/Durian/Endless pack grants (seeds). All four pass `maxItems = Inventory.HARD_MAX_ITEMS`; normal pickups/grants still refuse above 250. If a new over-soft-cap path is added, list it here.
