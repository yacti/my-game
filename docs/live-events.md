...
Panel sections → control plane: OwnerMessage/AlienMessage GO buttons → `SendGlobalMessage` (Owner/Alien preset; the GO button is the **only** submit path), LuckAmount CONFIRM → `SetAdminLuck` (preview in `LuckPreview`; the REMOVE button switches the row to removal mode — preview reads `LUCK REMOVED` and CONFIRM pushes `StopAdminLuck` instead), AdminEvents → `StartGlobalEvent("Alien")` / `StopGlobalEventsByEventId("Alien")`, PadToggle → `SetAdminPartyPad`, CrateDrops → `DropAdminCrate` (InfernoCrate x75 / SaturnCrate x250 / UFOCrate x1000, plus the rarity-pool SecretLuckyBlock / MythicLuckyBlock), Pinata → `SpawnPinata` (DEFAULT / GOLDEN, one pinata per press) and `ClearPinatas` (CLEAR), SprinkleGrowth START/STOP → `StartGlobalEvent("Sprinkle")` / `StopGlobalEventsByEventId("Sprinkle")`. Alien Start runs until Alien Stop, with a hidden 30-minute failsafe; Sprinkle behaves the same way. The same AdminEvents row also carries **ASTAND ON / ASTAND OFF** → `SetAlienStand(true/false)`, which is Mr Alien's stand and *not* the invasion: it is a durable boolean like the party pad, with no record, no duration and no failsafe, so it stays up until an operator turns it off. All panel commands land in the audit list with `operatorNote = "AdminPanel <name>"`.

## Mr Alien's stand

`SetAlienStand(true)` clones `ReplicatedStorage.Assets.AdminEvents.MrAlienEvent` into `Workspace.Map` on every server (`src/server/AlienStandService.luau`); `SetAlienStand(false)` destroys it. The template is authored in place, so the clone keeps its CFrames — there is no spawn marker to maintain. Booting servers pick the flag up from the same reconcile that carries the party pad, so a server that starts mid-event raises the stand at startup.

While the stand is up: the HUD EventButton swaps `Icon` for `AlienIcon` and the EVENT teleport button sends players to the stand's `AlienTeleport` brick instead of the bee camp (`HomeTeleportService`). Players deposit Alien-mutated food into the `AlienCrate` for tiered seed rewards (11 → Alien Egg, 22 → Alien Fungus, 44 → Eyeshroom; tiers loop). All three are EventOnly + Exclusive patch seeds carrying `ExclusiveDescription = "Alien Event"`, ordered so `PercentageOfBest` climbs with cost: 1.11 → 1.33 → 1.66. Progress lives in `profile.Data.AlienStand` and survives the stand coming down, so a returning player resumes mid-tier. Deposits are refused while the stand is down — unlike the always-on bee jar, the crate does not exist then.
...
**Offers are global with no messaging.** Each of the three slots draws with equal odds from its own pool (7/9/8 entries), chosen by hashing the visit's `startsAt` (`TravellingMerchant.OffersFor`). Because every server computes the same `startsAt`, every server worldwide shows the same three offers that hour — cross-server discussion works, and nothing is replicated to achieve it. The client derives its own cards the same way; only the player's *claims* are replicated (`profile.Data.TravellingMerchant`), since those are per-player. A new `InstanceId` wipes `Claimed` lazily, so each visit is a fresh three claims.

Trading consumes food from the backpack, cheapest-XP first, skipping favourites — so a trade never eats the mutated fruit a player was saving. Gates are the standard pair: `EventEligibility` (no tutorial, no pending offline progress, plot assigned) plus `isInventoryGameplayBlocked` (which additionally covers the inventory-restore window that `EventEligibility` does not). Every refusal raises a toast naming the reason. Multi-skip rewards keep their configured `Skip5m` identity and amount (`x2`/`x3`) on the card, then apply the combined 10/15-minute plot-time reduction atomically when claimed.

**Skip products are per slot.** `ShopBalance.TravellingMerchant.Skips` maps one developer product to each of the three visible slots; purchasing it grants whichever reward that slot rolled without consuming the requested food. `AssetValidator.validateShopProductIds` checks the configured IDs for collisions with every other product route. Setting a slot's ID to `0` deliberately hides only that Skip button and makes that slot trade-only.
...
Studio testing: `GameEventDevCommand` provides `merchant-offers` (reports this hour's three offers and how many of each request the player holds) and `grant-food` (bulk food for high-count trades).
...
-- Sprinkle: yactiNPC tours eligible farms boosting food XP. Runs until stopped,
-- with a hidden 30-minute failsafe (same shape as Alien).
local ok, r = _G.LE.StartGlobalEvent("Sprinkle", { commandId = "sprinkle-2026-07-30-a" }) print(ok, typeof(r) == "table" and r.instanceId or r)
print(_G.LE.StopGlobalEventsByEventId("Sprinkle"))

-- Studio has no DataStore, so drive the runner directly instead:
-- require(game.ServerScriptService.Server.GameEventService).StartManualEvent("Sprinkle")
-- ...or from an MCP/separate VM, via the dev attribute (see docs in src/server/dev/GameEventDev.luau):
-- Server:SetAttribute("GameEventDevCommand", HttpService:JSONEncode({ action = "start", eventId = "Sprinkle" }))
...
- **Pinatas.** `SpawnPinata(variant)` is **one-shot**, exactly like `DropAdminCrate`: every server that receives the broadcast spawns one pinata at a random free `Workspace.Map.PinataSpawn` slot, a server that misses the message skips it, and servers booting later get nothing. Nothing is durable — a restart loses them. The delta carries only the variant, never a slot, because occupancy differs per server. **Cap is 8** (one per authored spawn marker); a spawn with every slot full returns `no-free-slot`. Pinatas expire after **30 minutes**. Payout rules differ by ending: breaking one pays the **full** pool split by damage share; **expiry with damage** also breaks and pays out (so contributors are not wiped at 249/250); expiry with **zero** damage removes it silently; and `ClearPinatas()` is the kill switch — it removes every pinata and every uncollected drop with **no payout**. Rewards drop as owner-tagged physical pickups that must be walked over, and anything still on the ground after 60s is auto-granted to its owner. Grow-skip drops that land while the player has nothing growing are deliberately left on the ground rather than silently consumed. Players within 15 studs of a pinata get a welded stick (not a Tool — it must not disturb the equipped-tool slot that pet feeding reads); swings are server-validated at 1 HP each on a 0.45s cooldown, which is the only thing bounding damage rate. Unlike the plot events and admin crate drops, pinatas run **no `EventEligibility` gate** — the spawn toast, the stick, swings and drop collection all reach players still in the tutorial; a grant needs only a loaded profile, plus an assigned plot for the grow-skip kind. Reward counts split by largest remainder, rarest category first, with a 1%-of-max-HP eligibility floor; see `src/shared/Pinata.luau`.
- **Sprinkle.** `StartGlobalEvent("Sprinkle")` drops **yactiNPC** onto the island at `Workspace.Map.yactiNPCspawn` from 200 studs up, then walks him farm to farm at 20 studs/s. How many stops a farm gets is driven by **how much plot the player has bought**: at or below `singleStopMaxCells` (4) unlocked cells it is always exactly **1**, and above that it is a random **1–3** so repeat events do not play out identically. Within that budget the stops are placed by greedy maximum coverage over the food that is actually there, restricted to **unlocked grid cells**, each at least 24 studs from the others. A budget larger than the farm needs simply finishes early. Note the consequence: a 24-stud wave covers ~1,800 sq studs and a full plot is 14,080, so a large farm has **some crops left untouched** — that is intended, not a bug. At each stop he raises the `MagicWand`, waves it for **5 seconds**, and multiplies the XP of every patch slot fruit and tree ground pile within **24 studs** by **1.5x** — applied as 10 equal ramp steps so the fruit visibly swells (clients derive patch scale from the replicated XP anchor, so the growth needs no client code). Eligibility is identical to the UFO's (`EventEligibility`: no tutorial, no pending offline progress, plot assigned). **Sprinkle is turn-based: exactly one turn per player, never a revisit.** The queue holds up to 8, players who become eligible later append to the **end** of it in the order they first became valid targets, and the event **ends by itself once the queue drains** — after a 10-second grace in case someone qualifies right at the end (a new arrival restarts that grace). The 30-minute failsafe is only a ceiling; a normal run finishes long before it. Sprinkle Stop ends it early, applying whatever the in-flight wave still owes **in one write** so no farm is left holding a partial multiplier, and then he rises 200 studs over 6 seconds and fades out. Stop selection removes each target from the pool as soon as a stop covers it, so a turn's stops **partition** that farm's food and no crop is ever boosted twice. Every turn prints `[PlotSprinkleService] turn user=<id> cells=<c> budget=<b> stops=<s> eligibleTargets=<t>` followed by one `wave ... stop=<i>/<s> targets=<k> x1.50 (turn total <m>)` per stop; the same `user=` appearing in two different turns would mean the one-turn-per-player rule had broken. Food with a live steal reservation is skipped (its price was locked at reservation time). Sprinkle has no `HUD.TopBar.CurrentEvent` frame and is deliberately excluded from the top-bar carousel; players see toasts instead. The NPC is rendered entirely client-side — nothing about it crosses the network except a phase, two positions and two timestamps.
...
- **Alien HUD and music.** While Alien is active, `HUD.TopBar` always shows the Alien frame and `ADMIN PARTY NOW!`; concurrent weather/chaos records do not enter the display carousel. The client suppresses the regular random music playlist, plays `1836594449` → `134434412138800` → `139266399633943` → `140245756477343` → `139992996755484` continuously in order, then repeats from the first track. Regular music resumes after Alien winds down/stops.
