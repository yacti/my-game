...
Rolls are currently server-authoritative and purchased with in-game currency after a server-selected offer. `RollService` sends each reward's base `1 in n` chance text (luck weights the draw but never the displayed odds), but there is still no paid-random-item UI disclosure flow because rolls are not currently Robux-paid. Code does not reference `PolicyService.ArePaidRandomItemsRestricted`.
